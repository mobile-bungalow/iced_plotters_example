...
use iced_plotters::iced::{
    canvas, canvas::layer, executor, Application, Canvas, Command, Container, Element, Length,
    Settings,
};
use iced_plotters::plotters::drawing::IntoDrawingArea;
use iced_plotters::plotters::prelude::*;
use iced_plotters::PlotFrame;
use iced_plotters::Plottable;
...
#[derive(Debug)]
struct Example {
    cache: layer::Cache<Self>,
}

impl canvas::Drawable for Example {
    fn draw(&self, frame: &mut canvas::Frame) {
        self.draw_plot(PlotFrame(frame));
    }
}

impl Plottable for Example {
    fn draw_plot(&self, frame: iced_plotters::PlotFrame) {
        let data = get_data();
        let root = frame.into_drawing_area();
        ...
        chart
            .draw_series(
                data.iter().map(|x| {
                    CandleStick::new(parse_time(x.0), x.1, x.2, x.3, x.4, &GREEN, &RED, 15)
                }),
            )
            .unwrap();
    }
...
impl Application for Example {
    ...
    fn view(&mut self) -> Element<Message> {
        let canvas = Canvas::new()
            .width(Length::Units(750))
            .height(Length::Units(750))
